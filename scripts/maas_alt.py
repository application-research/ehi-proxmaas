...
# Write a copy of the VM details to a file
...
vm_details = json.load(args.vm_details)
for vm in vm_details:
    print(vm.mac)
    print(vm.name)
    # for mac in vm_details[vm]["mac"]:
    #     if mac in machine_mac_addresses:
    #         target_mac_address = mac

# For machine with this MAC
print(machine_mac_addresses[target_mac_address])
machine = client.machines.get(system_id=machine_mac_addresses[target_mac_address]["system_id"])
machine.hostname = "dev-test01"
